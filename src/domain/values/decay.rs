...
pub fn half_life_hours(category: &Category) -> f64 {
    match category {
        ...
        Category::CryptoSignal => 48.0,     // 2 days
        Category::TradingThesis => 120.0,   // 5 days
        Category::Catalyst => 96.0,         // 4 days
        Category::EconomicCalendar => 24.0, // 1 day (events pass quickly)
        Category::EarningsSignal => 72.0,   // 3 days

        // Medium-decaying: news and social signals
        Category::Newsletter => 168.0,      // 7 days
        Category::Social => 120.0,          // 5 days
        Category::SectorAnalysis => 168.0,  // 7 days
        Category::MacroRegime => 336.0,     // 14 days
        ...
        Category::Kalshi => 168.0,          // 7 days

        // Very slow: research and regulatory
        Category::ResearchNote => 720.0,    // 30 days
        Category::RegulatoryWatch => 720.0, // 30 days

        // Portfolio tracking decays slowly
        Category::PortfolioNote => 720.0,   // 30 days
        Category::RiskAlert => 168.0,       // 7 days
        Category::Trading => 168.0,         // 7 days
    }
}
...
mod tests {
    ...
    fn test_no_decay_for_fresh_entry() {
        ...
        // Should be very close to original (within float precision)
